...
class Main extends PluginBase implements Listener 
{
  ...
  public function onCMD(PlayerCommandPreprocessEvent $event) : void 
  {
		...
		if($cmd[0] !== "/") return;
		if($sender->hasPermission("levellimiter.bypass")) return;
		if($sender->hasPermission("levellimiter.bypass." . $sender->getWorld()->getFolderName())) return;
		$cmdo = trim($cmd, "/");
		if($sender->hasPermission("levellimiter.bypass." . $sender->getWorld()->getFolderName() . "." . explode(" ",$cmdo)[0])) return;
